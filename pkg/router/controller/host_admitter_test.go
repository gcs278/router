...
func (r routeStatusRecorder) RecordRouteNotDeprecated(route *routev1.Route) {
	deprecated := make(map[string]string)
	for key, reason := range r.deprecated {
		if key != r.rejectionKey(route) {
			deprecated[key] = reason
		}
	}
	r.deprecated = deprecated
}

func (r routeStatusRecorder) RecordRouteDeprecated(route *routev1.Route, reason, message string) {
	r.deprecated[r.rejectionKey(route)] = reason
}
