...
func (o *RouterSelection) Bind(flag *pflag.FlagSet) {
	...
	flag.BoolVar(&o.DeprecationValidation, "deprecation-validation", isTrue(env("DEPRECATION_VALIDATION", "true")), "If set, then an additional deprecation validation step is performed on all routes processed by this router. Defaults to true and enables the deprecation validation checks.")
